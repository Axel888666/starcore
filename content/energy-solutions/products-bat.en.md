---
title: "StarKernal BAT Energy Storage Battery"
description: "LiFePO4 battery, 6.0kWh/10.0kWh, cycle life ≥10000 times, IP66 protection"
weight: 3
---

<div class="energy-page-header">
  <div class="energy-breadcrumb"><a href="/energy-solutions/">Smart Energy</a> › Products › StarKernal BAT</div>
  <h1>StarKernal BAT Energy Storage Battery</h1>
  <p>LiFePO₄ Lithium Iron Phosphate　|　Modular Parallel Expansion</p>
</div>

![StarKernal BAT Energy Storage Battery Comparison](images/energy-bat-battery.svg)

## Product Models

<div class="energy-bat-compare">
  <div class="energy-bat-model">
    <h3>StarKernal BAT 6.0</h3>
    <div class="energy-bat-chemistry">LiFePO₄ Lithium Iron Phosphate</div>

| Parameter | Specification |
|-----------|--------------|
| Rated Capacity | **6.02 kWh** |
| Nominal Voltage | 314 Ah |
| Charge Power | 3000 W |
| Discharge Power | 4600 W |
| Cycle Life | **≥ 10000 Cycles** |
| Weight | 62 kg |
| Protection Rating | **IP66** |

  </div>
  <div class="energy-bat-model">
    <h3>StarKernal BAT 10.0</h3>
    <div class="energy-bat-chemistry">LiFePO₄ Lithium Iron Phosphate</div>

| Parameter | Specification |
|-----------|--------------|
| Rated Capacity | **9.04 kWh** |
| Nominal Voltage | 314 Ah |
| Charge Power | 4500 W |
| Discharge Power | 6900 W |
| Cycle Life | **≥ 10000 Cycles** |
| Weight | 78 kg |
| Protection Rating | **IP66** |

  </div>
</div>

## Core Advantages

### LiFePO₄ Chemistry
Mature and stable LiFePO₄ technology with excellent thermal stability, no risk of thermal runaway, far superior safety compared to ternary lithium batteries.

### Ultra-Long Cycle Life
Cycle life **≥ 10,000 times** — at one full cycle per day, this exceeds 27 years of use.

### IP66 All-Weather Protection
Full dust and strong water-jet protection, suitable for outdoor installation in any weather.

### Modular Flexible Expansion
Single or multiple units (up to 6) in parallel, flexibly scaling from 6kWh to 54kWh+.

### Multi-Layer Safety
Built-in BMS battery management system with cell-level overcharge, over-discharge, over-temperature, and short-circuit protection, plus AFCI arc detection.

[Back to Product Portfolio](/energy-solutions/) | [Contact Sales](/contact/)
